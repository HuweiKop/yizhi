		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<dataList.length;i++){
					...
					html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/manage/roleEdit?roleId="+dataList[i].id+"'>"+dataList[i].name+"<a/></td>"
					+"<td>"+isPublish(dataList[i].status)+"</td>"
					+"<td>"+dataList[i].lastOperator+"</td>"
					+"<td><a name='update' href='<%=request.getContextPath() %>/web/manage/roleEdit?roleId="+dataList[i].id+"' class=\"btn btn-link\">编辑</a></td></tr>";
				}
				$("#list").html(html);
				...
				<div class="col-md-10">
					...
					<div class=" pagination  ">
						<a class="btn btn-sm btn-primary" name="create"
							href="<%=request.getContextPath()%>/web/manage/roleEdit">
							新建</a>
					</div>
				</div>
			</div>
