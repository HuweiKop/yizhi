		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<dataList.length;i++){
					...
					html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/disease/indexEdit?indexId="+dataList[i].id+"'>"+dataList[i].name+"</a></td>"
					+"<td>"+isPublish(dataList[i].status)+"</td>"
					+"<td>"+dataList[i].lastOperator+"</td>"
					+"<td>"+showTime(dataList[i].updateTime)+"</td>"
					+"<td><a name='update' href='<%=request.getContextPath() %>/web/disease/indexEdit?indexId="
														+ dataList[i].id
														+ "' class=\"btn btn-link\">编辑</a>"
														+ "<a href='#' name='delete' onclick=\"deleteData("
														+ dataList[i].id
														+ ",'ind_id','t_dis_index',3)\" class=\"btn btn-link\">删除</a>";
...
<body>
	...
	<div id="page-wrapper">
		<div id="page-inner">
			<div class="row">
				<div class="col-md-12">
					<h1 class="page-head-line">指标列表</h1>
			...
			<div class="form-group col-md-12">
				<!--   修改这里搜索 -->
				<label class="control-label pull-left" for="name">指标名称</label>
				<div class="col-md-2">
				...
				<div class="col-md-2">
					<select class="form-control " id="orderBy">
						...
						<option value="ind_id">Id</option>
						<option value="convert(ind_name using gbk)">Name</option>
					</select>
				</div>
				...
				<div class="col-md-10">
					...
					<div class=" pagination  ">
						<a class="btn btn-sm btn-primary" name="create"
							href="<%=request.getContextPath()%>/web/disease/indexEdit">
							新建</a>
					</div>
					<div class="pagination">
