	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<dataList.length;i++){
				...
				html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/information/recommendEdit?infoId="+dataList[i].id+"'>"+dataList[i].title+"</a></td>"
				+"<td>"+isPublish(dataList[i].status)+"</td>"
				+"<td>"+dataList[i].lastOperator+"</td>"
				+"<td>"+showTime(dataList[i].updateTime)+"</td>"
				+"<td><a name='update' href='<%=request.getContextPath() %>/web/information/recommendEdit?infoId="
													+ dataList[i].id
													+ "' class=\"btn btn-link\">编辑</a>"
													+ "<a href='#' name='delete' onclick=\"deleteData("
													+ dataList[i].id
													+ ",'reco_id','t_info_recommend',10)\" class=\"btn btn-link\">删除</a></td></tr>";
										}
										$("#list").html(html);
...
<div id="page-wrapper">
		<div id="page-inner">
			<div class="row">
				<div class="col-md-12">
					<h1 class="page-head-line">推荐列表</h1>
			...
			<div class="form-group col-md-12">
				<!--   修改这里搜索 -->
				<label class="control-label pull-left" for="name">Title</label>
				<div class="col-md-2">
				...
				<div class="col-md-2">
					<select class="form-control " id="orderBy">
						...
						<option value="reco_id">Id</option>
						<option value="convert(title using gbk)">Name</option>
					</select>
				</div>
				...
				<div class="col-md-10">
					...
					<div class=" pagination  ">
						<a class="btn btn-sm btn-primary" name="create"
							href="<%=request.getContextPath()%>/web/information/recommendEdit">
							新建</a>
					</div>
					<div class="pagination">
