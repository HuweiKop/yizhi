...
<link href="../../assets/css/custom.css" rel="stylesheet" />
	...
	if("category=1".equals(par))	{
		editPage="surgeryEdit";
	}
	else if("category=2".equals(par)){
		editPage="chemotherapyEdit";
	}
	else if("category=3".equals(par)){
		editPage="radiotherapyEdit";
	}
	%>
<script type="text/javascript">
	$(function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<dataList.length;i++){
					...
					html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/disease/<%=editPage%>?therapeuticId="+dataList[i].id+"'>"+dataList[i].name+"</a></td>"
					+"<td>"+isPublish(dataList[i].status)+"</td>"
					+"<td>"+dataList[i].lastOperator+"</td>"
					+"<td>"+showTime(dataList[i].updateTime)+"</td>"
					+"<td><a name='update' href='<%=request.getContextPath() %>/web/disease/<%=editPage%>?therapeuticId="+dataList[i].id+"' class=\"btn btn-link\">编辑</a>"
					+"<a href='#' name='delete' onclick=\"deleteData("
					+ dataList[i].id
					+ ",'ther_id','t_dis_therapeutic',5)\" class=\"btn btn-link\">删除</a>";
					if(dataList[i].status!=1){
						html+="<a href='#' name='publish' onclick='publish("+dataList[i].id+")' class=\"btn btn-link\">发布</a>";
					}
					html+="</td></tr>";
				}
				$("#list").html(html);
...
<body>
	...
	<div id="page-wrapper">
		<div id="page-inner">
			<div class="row">
				<div class="col-md-12">
					<h1 class="page-head-line">治疗方法列表</h1>
			...
			<div class="form-group col-md-12">
				<!--   修改这里搜索 -->
				<label class="control-label pull-left" for="name">治疗方法名称</label>
				<div class="col-md-2">
				...
				<div class="col-md-2">
					<select class="form-control " id="orderBy">
						...
						<option value="ther_id">Id</option>
						<option value="convert(ther_name using gbk)">Name</option>
					</select>
				</div>
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="panel panel-default">
						<div class="panel-heading">治疗方法列表</div>
				...
				<div class="col-md-10">
					...
					<div class=" pagination  ">
						<a class="btn btn-sm btn-primary" name="create"
							href="<%=request.getContextPath()%>/web/disease/<%=editPage%>">
							新建</a>
					</div>
					<div class="pagination">
