		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<dataList.length;i++){
					...
					html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/information/informationEdit?infoId="+dataList[i].id+"'>"+dataList[i].title+"</a></td>"
					+"<td>"+isPublish(dataList[i].status)+"</td>"
					+"<td>"+dataList[i].lastOperator+"</td>"
					+"<td>"+showTime(dataList[i].updateTime)+"</td>"
					+"<td><a name='update' href='<%=request.getContextPath() %>/web/information/informationEdit?infoId="
														+ dataList[i].id
														+ "' class=\"btn btn-link\">编辑</a>"
														+ "<a href='#' name='delete' onclick=\"deleteData("
														+ dataList[i].id
														+ ",'info_id','t_info_information',10)\" class=\"btn btn-link\">删除</a></td></tr>";
											}
											$("#list").html(html);
...
<body>
	...
	<div id="page-wrapper">
		<div id="page-inner">
			<div class="row">
				<div class="col-md-12">
					<h1 class="page-head-line">信息列表</h1>
			...
			<div class="form-group col-md-12">
				...
				<div class="col-md-2">
					<select class="form-control " id="orderBy">
						...
						<option value="convert(title using gbk)">Title</option>
					</select>
				</div>
