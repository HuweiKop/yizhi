			...
			$.ajax({
				...
				success : function(data) {
					...
					for(var i=0;i<dataList.length;i++){
						...
						html+="<tr><td>"+dataList[i].id+"</td><td><a href='<%=request.getContextPath() %>/web/medicine/medicineEdit?medicineId="+dataList[i].id+"'>"+dataList[i].name+"</a></td>"
						+"<td>"+isPublish(dataList[i].status)+"</td>"
						+"<td>"+dataList[i].lastOperator+"</td>"
						+"<td>"+showTime(dataList[i].updateTime)+"</td>"
						+"<td><a name='update' href='<%=request.getContextPath() %>"
														+ "/web/medicine/medicineEdit?medicineId="
														+ dataList[i].id
														+ "' class=\"btn btn-link\">编辑</a>"
														+ "<a href='#' name='delete' onclick=\"deleteData("
														+ dataList[i].id
														+ ",'med_id','t_med_medicine',8)\" class=\"btn btn-link\">删除</a>";
...
<body>
	...
	<div id="page-wrapper">
		<div id="page-inner">
			<div class="row">
				<div class="col-md-12">
					<h1 class="page-head-line">药品列表</h1>
			...
			<div class="form-group col-md-12">
				<!--   修改这里搜索 -->
				<label class="control-label pull-left" for="name">药品名称</label>
				<div class="col-md-2">
				...
				<div class="col-md-2">
					<select class="form-control " id="orderBy">
						...
						<option value="med_id">Id</option>
						<option value="convert(med_name using gbk)">Name</option>
					</select>
				</div>
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="panel panel-default">
						<div class="panel-heading">药品列表</div>
				...
				<jsp:include page="../common/pageInfo.jsp"></jsp:include>
					...
					<div class=" pagination  ">
						<a class="btn btn-sm btn-primary" name="create"
							href="<%=request.getContextPath()%>/web/medicine/medicineEdit"> 新建</a>
					</div>
					<div class="pagination"><input type="button" onclick="gotoPage(1)" value="首页" class="btn btn-sm btn-default"></div>
				</div>
